/**
  ...
  $Last Modified: 2022/12/23 14:12:06
...
var coolants = [
  {id: COOLANT_FLOOD, on: 8, off: 28},
  {id: COOLANT_MIST, on: 25, off: 9},
  {id: COOLANT_THROUGH_TOOL, on: 7, off: 27},
  {id: COOLANT_AIR, on: 85, off: 86},
...
function defineMachine() {
  if (false) { // note: setup your machine here
    //var aAxis = createAxis({coordinate:0, table:true, axis:[1, 0, 0], range:[-120.0001, 120.0001], preference: getProperty("preferTilt"), tcp:true});
    var bAxis = createAxis({coordinate:1, table:true, axis:[0, 1, 0], range:[-90.0001, 110.0001], preference:1, tcp:true});
...
function setTCP(tcp) {
  if (typeof currentSection == "undefined" ||
    (getProperty("usePlane") == "none" && !tcpIsSupported)) {
    return;
  }
  if (tcp) {
...
function getCoolantCodes(coolant, isGunDrilling) {
  ...
  if (coolant == currentCoolantMode && !forceCoolantCode) {
    return undefined; // coolant is already active
  }
  forceCoolantCode = false;
